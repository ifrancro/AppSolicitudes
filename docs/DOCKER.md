...
## Decisión pendiente: base de datos de las pruebas

`AppSolicitudesAdmin/phpunit.xml` usa hoy SQLite en memoria. Es rápido, pero no
prueba el comportamiento real de PostgreSQL (tipos `jsonb`, restricciones,
diferencias de ordenación). El contenedor de PostgreSQL ya crea una base
`appsolicitudes_test` en su primer arranque.

Cuando se decida usarla, basta con cambiar en `phpunit.xml`:

```xml
<env name="DB_CONNECTION" value="pgsql"/>
<env name="DB_DATABASE" value="appsolicitudes_test"/>
<env name="DB_HOST" value="postgres"/>
```

Recomendación: hacer el cambio antes de escribir consultas que dependan de
